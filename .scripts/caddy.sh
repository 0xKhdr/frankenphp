...
create_log_directories() {
    ...
    mkdir -p docker-compose/logs/caddy
    mkdir -p docker-compose/logs/php-fpm
}

# Function to build and start containers
start_containers() {
    print_status "Building and starting Caddy containers..."
...
run_laravel_setup() {
    ...
    for cmd in "${commands[@]}"; do
        docker compose -f docker-compose-caddy.yml exec -T php $cmd
    done
}
...
clean_logs() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        rm -rf docker-compose/logs/caddy/*
...
exec_command() {
    ...
    docker compose -f docker-compose-caddy.yml exec php "$@"
}

# Function to handle 'up' command
up_command() {
    print_header "Starting Caddy Stack"
    check_env_file
    create_log_directories
    start_containers
    wait_for_services
    ...
    print_status "Caddy stack is ready!"
...
down_command() {
    print_header "Stopping Caddy Stack"
    stop_containers
    remove_volumes
    clean_logs
    print_status "Caddy stack stopped!"
}

# Function to handle 'stress' command
stress_command() {
    print_header "Stress Testing Caddy Stack"
    run_stress_test "$@"
...
exec_command_handler() {
    print_header "Executing Command in Caddy Stack"
    check_php_service
    shift
    if [ $# -eq 0 ]; then
